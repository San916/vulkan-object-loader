...
bool MD5Model::LoadModel(const std::string& filename)
{
    ...
    {
        cout << "MD5Model::LoadModel: Failed to find file: " << filename << endl;
        return false;
    }
        ...
        else if (param == "mesh")
        {
            ...
            while (param != "}")  // Read until we get to the '}' character
            {
                if (param == "shader")
                {
                    ...
                    mesh.m_TexID = 0; //???????????????????????????????????????????????????????? under construction
...
//                        vert.m_Tex0.y = 1.0f - vert.m_Tex0.y; // chai?????????????????????????????????????????????????? inverse y texture coord
                ...
                else if (param == "numtris")
                {
                    ...
                    for (int i = 0; i < numTris; ++i)
                    {
                        ...
                        mesh.m_IndexBuffer.push_back((std::uint32_t)tri.m_Indices[0]);
                        mesh.m_IndexBuffer.push_back((std::uint32_t)tri.m_Indices[1]);
                        mesh.m_IndexBuffer.push_back((std::uint32_t)tri.m_Indices[2]);
                    }
                }
